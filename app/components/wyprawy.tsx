...
const oferty = [
  {
    id: "lodz",
    tytul: "Wyprawy z Łodzi",
    ikona: <Anchor className="text-blue-500" size={24} />, // Zmniejszono z 32
    opis: "Pełny profesjonalizm na falach Bałtyku i estuariach rzek.",
    cechy: ["Sprzęt w cenie", "Paliwo w cenie"], // Skrócono listę na mobile
    img: "/lodz.jpg",
    cena: "od 800 zł"
  },
  {
    id: "brzeg",
    tytul: "Przewodnik Brzegowy",
    ikona: <TreePine className="text-green-500" size={24} />,
    opis: "Trekking wzdłuż dzikich odcinków Regi i Parsęty.",
    cechy: ["Lokalna wiedza", "Analiza rynien"],
    img: "/lodz.jpg",
    cena: "od 400 zł"
  },
  {
    id: "full",
    tytul: "Full Trip",
    ikona: <Map className="text-red-500" size={24} />,
    opis: "Kompleksowa organizacja: nocleg, wyżywienie i łowienie.",
    cechy: ["Nocleg", "Wyżywienie"],
    img: "/lodz.jpg",
    cena: "od 2200 zł"
  }
];
...
export default function Wyprawy() {
  return (
    ...
    <section id="wyprawy" className="relative py-10 px-4 md:py-24 md:px-6 overflow-hidden bg-transparent">
      <div className="max-w-7xl mx-auto">
        ...
        >
          {/* Nagłówek: text-5xl -> text-2xl */}
          <h2 className="text-2xl md:text-7xl font-black uppercase tracking-tighter text-white mb-3">
            Profesjonalne <br /><span className="text-blue-600">Wyprawy</span>
          </h2>
          <p className="text-zinc-500 max-w-xl mx-auto text-xs md:text-lg italic font-serif opacity-80">
            "Nie sprzedajemy tylko czasu. Sprzedajemy lata doświadczeń."
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
          {oferty.map((oferta, index) => (
            <motion.div
              ...
              // Wysokość: h-[600px] -> h-[350px] na mobile
              className="group relative h-[350px] md:h-[600px] bg-zinc-900 rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden border border-white/5"
            >
              <div className="absolute inset-0 w-full h-full">
                <img 
                  src={oferta.img} 
              ...
              {/* Treść: p-8 -> p-5 na mobile */}
              <div className="absolute inset-0 p-5 md:p-8 flex flex-col justify-end z-10">
                <div className="mb-2">{oferta.ikona}</div>
                <h3 className="text-xl md:text-3xl font-bold text-white uppercase mb-1 tracking-tight leading-none">
                  {oferta.tytul}
                </h3>
                <p className="text-zinc-300 text-[10px] md:text-sm leading-tight mb-4 italic line-clamp-2 md:line-clamp-none">
                  {oferta.opis}
                </p>

                {/* Cechy widoczne tylko na większych ekranach lub w bardzo uproszczonej formie */}
                <ul className="space-y-1 mb-4 hidden md:block">
                  {oferta.cechy.map((cecha) => (
                    <li key={cecha} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-white/70">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-white/10">
                  <span className="text-blue-400 font-black text-lg leading-none">{oferta.cena}</span>
                  <a href="/kontakt" className="p-2.5 bg-white text-black rounded-full hover:bg-blue-600 hover:text-white transition-all">

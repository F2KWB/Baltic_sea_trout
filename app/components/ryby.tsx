...
export default function ArchiwumOkazow() {
  const archiwalne = [
    { id: 1, tytul: "Troć z Regi", kat: "Rzeka", opis: "75 cm żywego srebra złowione na autorski wobler.", img: "/troc_wobler.jpg" },
    { id: 2, tytul: "Srebrniak", kat: "Morze", opis: "Zimowy połów z plaży w ekstremalnych warunkach.", img: "/troc_widok.jpg" }
  ];
  ...
  return (
    <section className="py-2 px-2">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="text-[10px] font-light uppercase tracking-widest">Archiwum</h2>
      </div>

      <div className="flex flex-col gap-2">
        {archiwalne.map((ryba) => (
          <div key={ryba.id} className="flex flex-row bg-zinc-900/40 rounded-lg overflow-hidden border border-white/5 h-20">
            {/* LEWA: ZDJĘCIE (30%) */}
            <div className="w-[30%] shrink-0">
              <img src={ryba.img} className="w-full h-full object-cover" />
            </div>

            {/* PRAWA: TEKST (70%) */}
            <div className="w-[70%] p-2 flex flex-col justify-center">
              <span className="text-blue-500 text-[6px] uppercase font-black">{ryba.kat}</span>
              <h3 className="text-[10px] font-bold uppercase text-white leading-none mt-0.5">{ryba.tytul}</h3>
              <p className="text-zinc-500 text-[8px] leading-tight mt-1 line-clamp-2 italic font-serif">{ryba.opis}</p>
            </div>
          </div>
        ))}
      </div>

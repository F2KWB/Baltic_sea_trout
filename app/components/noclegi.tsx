...
export default function Noclegi() {
  ...
  const miejsca = [
    {
      ...
      kategoria: "Nad rzeką",
      lokalizacja: "Dorzecze Regi",
      opis: "Tradycyjna chata z bezpośrednim dostępem do najlepszych miejscówek trociowych.",
      cena: "150 zł / noc",
      img: "/nogi.jpg",
      tagi: ["Parking", "WiFi"]
    },
    {
      ...
      kategoria: "Nad morzem",
      lokalizacja: "Kołobrzeg",
      opis: "Apartamenty przygotowane specjalnie dla spinningistów brzegowych.",
      cena: "220 zł / noc",
      img: "/nogi.jpg",
      tagi: ["Widok", "Kuchnia"]
    }
  ...
  return (
    // Zmniejszono py-24 na py-10 oraz px-4
    <section id="noclegi-info" className="py-10 px-4 text-white scroll-mt-24 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col mb-8 gap-6">
          <motion.div
            ...
            className="px-1"
          >
            {/* Zmniejszono text-4xl na text-2xl */}
            <h2 className="text-2xl md:text-6xl font-black uppercase tracking-tighter leading-none">
              Baza <span className="text-blue-500">Noclegowa</span>
            </h2>
            <p className="text-zinc-500 mt-2 text-xs md:text-base">
              Sprawdzone miejsca przyjazne wędkarzom.
            </p>
          </motion.div>

          {/* FILTRY - Bardziej kompaktowe py-3 i mniejszy text */}
          <div className="flex flex-row flex-wrap md:flex-nowrap gap-1.5 w-full">
            {['Wszystkie', 'Nad rzeką', 'Nad morzem'].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`
                  flex-1 md:flex-none px-3 py-3 rounded-xl text-[9px] font-black uppercase tracking-wider transition-all border
                  ${filter === f 
        ...
        {/* SIATKA NOCLEGÓW - Zmniejszono gap-8 na gap-4 */}
          ...
          <AnimatePresence mode="popLayout">
            {filtrowaneMiejsca.map((miejsce) => (
              <motion.div
                ...
                className="group relative bg-zinc-900 rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden border border-white/10"
              >
                {/* ZDJĘCIE - Drastycznie obniżone h-64 na h-40 na mobile */}
                <div className="h-40 md:h-64 overflow-hidden relative">
                  <img src={miejsce.img} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt={miejsce.nazwa} />
                  <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 text-[8px] font-bold uppercase">
                    {miejsce.kategoria}
                  </div>
                </div>

                {/* TREŚĆ - Zmniejszono p-8 na p-4 */}
                <div className="p-4 md:p-10">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-2 mb-4">
                    <div>
                      {/* Zmniejszono text-2xl na text-lg */}
                      <h3 className="text-lg md:text-2xl font-black uppercase tracking-tight">{miejsce.nazwa}</h3>
                      <div className="flex items-center gap-1 text-blue-500 mt-1">
                        <MapPin size={12} />
                        <span className="text-[10px] font-bold uppercase">{miejsce.lokalizacja}</span>
                      </div>
                    </div>
                    {/* Cena jako mniejszy poziomy pasek na mobile */}
                    <div className="bg-white/5 px-3 py-1.5 rounded-lg border border-white/5 flex flex-row sm:flex-col items-center gap-2 sm:gap-0">
                      <p className="text-[8px] text-zinc-500 uppercase font-black">Cena:</p>
                      <p className="text-sm md:text-lg font-black text-white">{miejsce.cena}</p>
                    </div>
                  </div>

                  <p className="text-zinc-400 text-[11px] md:text-sm leading-snug mb-4 line-clamp-2 md:line-clamp-none">
                    {miejsce.opis}
                  </p>

                  {/* UDOGODNIENIA - Mniejszy tekst i padding */}
                  <div className="flex flex-wrap gap-1.5 pt-4 border-t border-white/5 text-[8px] md:text-[9px]">
                    {miejsce.tagi.map(tag => (
                      <span key={tag} className="bg-white/5 px-2 py-0.5 rounded text-zinc-400 font-bold uppercase border border-white/5">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                {/* NAKŁADKA - Uproszczona na mobile (tylko na desktopie widoczna po hover) */}
                <div className="absolute inset-0 bg-blue-600/95 hidden md:flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-all p-8 text-center">
                  <h4 className="text-3xl font-black uppercase mb-6 text-white leading-tight">Sprawdź<br/>dostępność</h4>
                  <button className="px-10 py-5 bg-white text-black font-black uppercase text-xs rounded-full">Otwórz</button>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>

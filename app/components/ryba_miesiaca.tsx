...
export default function RybaMiesiacaHero() {
  return (
    // Zmniejszono marginesy sekcji na mobile: py-10 -> py-4
    <section id="okazy" className="relative py-4 md:py-20 bg-transparent">
      <div 
        // Drastycznie zmniejszono wysokość na mobile: h-[500px] -> h-[320px]
      ...
      >
        ...
        <motion.div 
          ...
          className="relative z-10 text-center px-4 w-full"
        >
          {/* Badge: mniejszy padding i font */}
          <span className="inline-block border border-blue-600/50 bg-black/40 backdrop-blur-sm text-blue-400 px-4 py-1 rounded-full font-bold uppercase tracking-[0.2em] text-[8px] md:text-xs mb-3">
            Aktualne Trofeum
          </span>
          
          {/* RESPONSYWNY NAGŁÓWEK - zmniejszono skale jednostek vw */}
          <h1 className="flex flex-col items-center justify-center leading-[0.85] tracking-tighter uppercase font-black">
            <span className="text-[10vw] md:text-[10vw] lg:text-[7.5rem]">
              Ryba
            </span>
            <span
              className="text-transparent border-t border-b border-white py-1 px-4 italic text-[8.5vw] md:text-[8vw] lg:text-[6.5rem] mt-1"
              style={{ WebkitTextStroke: "1px white" }}
            >
              Miesiąca
            </span>
          ...
          {/* Opis: mniejszy font text-xs i krótszy margines mt-4 */}
          <p className="mt-4 md:mt-10 text-white text-xs md:text-xl max-w-2xl mx-auto leading-relaxed italic font-serif opacity-90 px-4 line-clamp-3">
            "Potężny samiec troci wędrownej złowiony o świcie w dorzeczu
            Parsęty. Ryba po krótkiej sesji wróciła do wody."
          </p>
        </motion.div>
      </div>
    </section>

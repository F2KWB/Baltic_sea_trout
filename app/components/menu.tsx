...
//     { name: "Rękodzieło", href: "/#rekodzielo" },
...
//             <a href="/#rekodzielo" className={linkStyle}>Rękodzieło</a>
...
import Link from "next/link";
import { motion, AnimatePresence } from "framer-motion";
import { Map, BedDouble, X, ChevronDown } from "lucide-react";

export default function Menu() {
  const [isOpen, setIsOpen] = useState(false);
  const [hoveredItem, setHoveredItem] = useState<string | null>(null);

  // Zmniejszono paddingi i fonty dla wersji desktopowej, aby menu było smuklejsze
  const linkStyle = "px-2 xl:px-4 py-1.5 border border-white/10 rounded-full hover:border-white hover:bg-white/10 transition-all duration-300 text-white whitespace-nowrap flex items-center gap-1 xl:gap-2 cursor-pointer text-[9px] xl:text-[10px]";
  const contactStyle = "px-4 xl:px-5 py-1.5 bg-blue-600 text-white rounded-full hover:bg-blue-500 transition-all shadow-lg shadow-blue-600/20 font-bold uppercase text-[9px] xl:text-[10px]";
  ...
  return (
    <>
      {/* Zmniejszono wysokość h-[80px] -> h-[60px] na mobile */}
      <nav className="fixed top-0 left-0 w-full z-[9999] h-[60px] md:h-[80px] bg-black/95 backdrop-blur-md border-b border-white/5 flex items-center">
        <div className="max-w-[1400px] w-full mx-auto px-4 md:px-6 flex justify-between items-center">
          
          {/* LOGO - Zmniejszono font i wysokość linii */}
          <Link href="/" className="text-sm md:text-xl font-black tracking-tighter text-white uppercase leading-[1] md:leading-normal shrink-0">
            BALTIC <br className="md:hidden" /> 
            <span className="whitespace-nowrap text-blue-500 md:text-white">SEA TROUT</span>
          </Link>
          
          {/* DESKTOP MENU */}
          <div className="hidden xl:flex gap-2 text-[10px] font-bold uppercase tracking-widest items-center">
            {/* ... (Twoje linki pozostają w linkStyle) */}
            <a href="/#o-nas" className={linkStyle}>O nas</a>
            <a href="/#okazy" className={linkStyle}>Okazy</a>
            <a href="/#tworcy" className={linkStyle}>Twórcy</a>
            <Link href="/kontakt" className={contactStyle}>Kontakt</Link>
          ...
          {/* MOBILE BUTTON - Mniejszy format */}
          <button 
            onClick={() => setIsOpen(!isOpen)} 
            className="xl:hidden text-white border border-white/10 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest bg-white/5 active:scale-95 transition-all"
          >
            {isOpen ? <X size={16} /> : "Menu"}
          </button>
        </div>
      </nav>

      {/* SIDE DRAWER - Skondensowany */}
      <AnimatePresence>
        {isOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOpen(false)} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[10000] xl:hidden" />
            <motion.div 
              initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
              transition={{ type: "spring", damping: 25, stiffness: 200 }}
              className="fixed top-0 right-0 h-screen w-[260px] bg-zinc-950 z-[10001] xl:hidden border-l border-white/10 flex flex-col"
            >
              {/* Zmniejszono wysokość nagłówka w drawerze */}
              <div className="h-[60px] flex items-center px-6 border-b border-white/5">
                <span className="text-zinc-500 font-bold text-[8px] tracking-[0.3em] uppercase">Nawigacja</span>
              </div>
              ...
              <div className="flex-grow p-4 overflow-y-auto">
                <div className="grid grid-cols-2 gap-2">
                  {fullNavigation.map((item) => (
                    <Link 
                      ...
                      href={item.href}
                      onClick={() => setIsOpen(false)}
                      className={`flex items-center justify-center gap-2 p-3 rounded-xl text-[8px] font-black uppercase tracking-tight text-center transition-all ${
                        item.blue 
                        ? "bg-blue-600 text-white shadow-lg shadow-blue-600/20" 
                        : "bg-white/5 text-zinc-300 border border-white/5"
                      }`}
                ...
                <Link href="/kontakt" onClick={() => setIsOpen(false)} className="mt-2 flex items-center justify-center p-4 bg-white text-black rounded-xl text-[9px] font-black uppercase tracking-widest">
                  Kontakt
                </Link>
              </div>

              <div className="p-6 border-t border-white/5 text-center">
                <p className="text-zinc-700 text-[7px] uppercase font-bold tracking-[0.4em]">© Baltic Sea Trout</p>
              </div>
            </motion.div>
          </>
        )}
